...
def get_random_data_with_Mosaic(annotation_line, input_shape, max_boxes=50, hue=.1, sat=1.5, val=1.5):
    ...
    for line in annotation_line:
        ...
        # 对输入进来的图片进行缩放
        new_ar = w / h
        scale = rand(int(scale_low), int(scale_high))
        if new_ar < 1:
            nh = int(scale * h)
            nw = int(nh * new_ar)
        else:
...
def get_random_data(annotation_line, input_shape, max_boxes=50, jitter=.3, hue=.1, sat=1.5, val=1.5):
    ...
    # 对图像进行缩放并且进行长和宽的扭曲
    new_ar = w / h * rand(int(1 - jitter), int(1 + jitter)) / rand(int(1 - jitter), int(1 + jitter))
    scale = rand(int(.25), 2)
    if new_ar < 1:
        nh = int(scale * h)
        nw = int(nh * new_ar)
    ...
    # 色域扭曲
    hue = rand(int(-hue), int(hue))
    sat = rand(1, int(sat)) if rand() < .5 else 1 / rand(1, int(sat))
    val = rand(1, int(val)) if rand() < .5 else 1 / rand(1, int(val))
    x = rgb_to_hsv(np.array(image) / 255.)
    x[..., 0] += hue
    x[..., 0][x[..., 0] > 1] -= 1
    x[..., 0][x[..., 0] < 0] += 1
